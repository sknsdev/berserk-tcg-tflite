...
# -*- coding: utf-8 -*-
"""
Скрипт для организации карт в подпапки по префиксам
Автоматически создает структуру папок на основе первого значения до символа подчеркивания
"""
...
def organize_cards(cards_dir='./cards'):
    """
    Организует карты в подпапки по префиксам (первое значение до символа подчеркивания)
    ...
    # Анализируем названия файлов и группируем по префиксам
    class_groups = {}
    
    for image_file in image_files:
        filename = image_file.stem  # Имя без расширения
        
        # Определяем префикс как первое значение до символа подчеркивания
        if '_' in filename:
            prefix = filename.split('_')[0]
        else:
            # Если нет символа подчеркивания, используем "no_prefix"
            prefix = 'no_prefix'
        
        if prefix not in class_groups:
            class_groups[prefix] = []
        class_groups[prefix].append(image_file)
    
    print(f"\n📊 Обнаружено {len(class_groups)} префиксов:")
    ...
    for class_name, files in class_groups.items():
        ...
        print(f"\n📂 Обрабатываем префикс '{class_name}'...")
    ...
    print(f"📂 Создано {len(class_groups)} папок по префиксам")
...
def main():
    ...
    Главная функция
    """
    print("🃏 ОРГАНИЗАЦИЯ КАРТ ПО ПРЕФИКСАМ")
    print("=" * 40)
    ...
    print("Хотите организовать их в подпапки по префиксам? (y/n): ", end="")
